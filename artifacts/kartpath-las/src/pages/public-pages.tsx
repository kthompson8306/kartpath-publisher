          ...
          {submitted ? (
            ...
            <>
              ...
              <form className="nominate-form" onSubmit={(e) => {
                ...
                <label className="full">Street Address <span style={{ fontWeight: 400, opacity: 0.65 }}>(optional)</span><input type="text" name="address" placeholder="123 Main Street, Senoia GA 30276" /></label>
...
export function Directory() {
  ...
  return <PageShell seo={{ title: 'Business Directory — Life Around Senoia', description: 'Find published storefronts, services, restaurants, and local businesses across Senoia, Georgia.', path: '/directory' }}>
    <PageHero kicker="Business Directory" title={<>Every business serving<br />Senoia and the surrounding community</>}>Published businesses and services from Life Around Senoia.</PageHero>
    ...
    <section><div className="wrap"><div className="dir-controls"><label className="dir-search"><Search size={17} /><input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search businesses or categories..." aria-label="Search businesses" /></label><div className="filter-pills">{categories.map((cat) => <button type="button" className={`filter-pill ${category === cat ? 'active' : ''}`} onClick={() => setCategory(cat)} key={cat}>{cat}</button>)}</div></div><PublicContentState query={contentQuery} emptyMessage="No businesses are published right now." /><div className="biz-grid">{[...filtered].sort((a, b) => ((a.listingTier === 'premium' ? 0 : 1) - (b.listingTier === 'premium' ? 0 : 1))).map((item) => {const isPremium = item.listingTier === 'premium'; return (<article className={`biz-card${isPremium ? ' biz-card--premium' : ''}`} key={item.id} data-testid={`public-published-${item.slug}`}>{isPremium && item.coverUrl && (<div className="biz-logo-wrap"><img src={item.coverUrl} alt={`${item.title} logo`} className="biz-logo-img" loading="lazy" /></div>)}<span className="cat">{item.details.category ?? 'Business Listing'}</span>{isPremium && <span className="biz-premium-badge">★ Premium</span>}<h2>{item.title}</h2>{item.details.address && <p className="biz-address">{item.details.address}</p>}<p className="contact">{item.details.phone ?? item.details.website ?? ''}</p>{isPremium && item.businessDescription && <p className="biz-desc">{item.businessDescription}</p>}{(item.details.facebook_url || item.details.instagram_url) && (<div className="biz-social">{item.details.facebook_url && <a href={item.details.facebook_url} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="biz-social-icon"><FaFacebook /></a>}{item.details.instagram_url && <a href={item.details.instagram_url} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="biz-social-icon biz-social-icon--ig"><FaInstagram /></a>}</div>)}</article>)})}</div>{!contentQuery.isPending && !contentQuery.isError && contentQuery.data?.length && filtered.length === 0 ? <div className="empty-state">No businesses match that search. Try another category or phrase.</div> : null}</div></section>
